...
# =====================================================================@001031=
# 1. podnaloga
# Sestavite funkcijo `vsebuje(niz, podniz)`, ki vrne `True`, če `niz`
# vsebuje `podniz` kot podniz.
# =============================================================================
def vsebuje(niz,podniz):
    if podniz in niz:
        return True
    else:
        return False
# =====================================================================@001032=
# 2. podnaloga
# Sestavite funkcijo `vsebuje_po_kosih(niz, podniz)`, ki vrne `True`, če in
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            Check.equal("vsebuje_po_kosih('milnica drazi vaso kozo', 'mladi voz')", True)
            Check.equal("vsebuje_po_kosih('otorinolaringolog', 'torino')", True)
            Check.equal("vsebuje_po_kosih('otorinolaringolog', 'milano')", False)
            Check.equal("vsebuje_po_kosih('otorinolaringolog', 'trololo')", True)
            for i in range(50):
                for j in range(20):
                    Check.secret(vsebuje_po_kosih(str(i), str(j)))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
