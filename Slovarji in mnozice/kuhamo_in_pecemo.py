...
def pomnozi(recept,faktor):
    ...
    for sestavina in recept:
        nov[sestavina] = recept[sestavina]* faktor
    return nov
# =====================================================================@001377=
# 2. podnaloga
...
def ali_imamo_sestavine(recept,shramba):
    for sestavina in recept:
        shramba[sestavina] = shramba.get(sestavina,0)
...
def kaj_moramo_se_kupiti(recept,shramba):
    if ali_imamo_sestavine(recept,shramba)==True:
        return {}
    nov={}
    for sestavina in recept:
        shramba[sestavina]=shramba.get(sestavina,0)
        if recept[sestavina] >  shramba[sestavina]:
            nov[sestavina]= recept[sestavina]-shramba[sestavina]
    return nov
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            ...
            Check.equal("kaj_moramo_se_kupiti({'jajca': 3, 'moka': 500}, {'moka': 100})", {'jajca': 3, 'moka': 400})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
