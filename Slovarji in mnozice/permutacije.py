...
# Na primer permutacijo, ki slika $1$ v $3$, $3$ v $1$, število $2$ pa
# pusti pri miru, zapišemo s slovarjem `{1: 3, 2: 2, 3: 1}`.
# =====================================================================@001379=
# 1. podnaloga
# Sestavite funkcijo `slika(permutacija, x)`, vrne pa sliko števila `x`
...
def slike(permutacija,x,n):
    zaporedje=[x]
    while len(zaporedje) < n+1:
        zaporedje.append(slika(permutacija,x))
...
def cikli(permutacija):
    cikli=[]
    v_ciklu=set()
    for kljuc in permutacija:
        if kljuc not in v_ciklu:
            x=cikel(permutacija,kljuc)
            cikli.append(x)
            v_ciklu.update(x)
    return cikli
...
def je_permutacija(slovar):
    ...
    for kljuc, vrednost in slovar.items():
        if slovar.get(vrednost,0)==0 or vrednost in sez:
            return False
        sez.append(vrednost)
    for i in range(1,max(sez)):
        if not i in sez:
            return False
    return True
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            ...
            Check.equal('je_permutacija({1: 3, 3: 4, 4: 1})', False)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
