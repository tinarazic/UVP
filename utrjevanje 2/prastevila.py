...
def prvo_prastevilo_z_vsoto_stevk_vsaj(n):
    p=2
    while vsota_stevk(p)<n:
        p=naslednje_prastevilo(p)
    return p
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            ...
            Check.equal('je_prastevilo(23)', True)
            Check.equal('je_prastevilo(42)', False)
            Check.equal('je_prastevilo(49)', False)
            for i in range(1, 100):
                Check.secret(je_prastevilo(i), i)
        except:
    ...
    if Check.part():
        try:
            for i in range(50, 100):
                Check.secret(naslednje_prastevilo(i), i)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    if Check.part():
        try:
            Check.equal('prvo_prastevilo_z_vsoto_stevk_vsaj(8)', 17)
            Check.equal('prvo_prastevilo_z_vsoto_stevk_vsaj(10)', 19)
            Check.equal('prvo_prastevilo_z_vsoto_stevk_vsaj(12)', 59)
            Check.equal('prvo_prastevilo_z_vsoto_stevk_vsaj(35)', 8999)
            Check.equal('prvo_prastevilo_z_vsoto_stevk_vsaj(36)', 29989)
            for i in range(10, 20):
                Check.secret(prvo_prastevilo_z_vsoto_stevk_vsaj(i))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
