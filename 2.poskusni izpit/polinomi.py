...
# Polinomi
#
# Polinome predstavimo s seznamom koeficientov, pri čemer element z indeksom $i$
# predstavlja koeficient ob $x^i$ (med drugim torej prazni seznam predstavlja
# ničelni polinom).
# =====================================================================@001357=
# 1. podnaloga
# Napišite funkcijo `odstrani_odvecne_nicle(koeficienti)`, ki vrne seznam
# `koeficienti`, v katerem smo na koncu odstranili odvečne ničle.
# 
...
#     []
# =============================================================================
def odstrani_odvecne_nicle(koeficienti):
    sez=[]
    for i in range(len(koeficienti)):
        if koeficienti[i]!=0:
            sez.append(koeficienti[i])
    return sez
# =====================================================================@001358=
# 2. podnaloga
# Napišite funkcijo `odvod_polinoma(polinom, n)`, ki za dani polinom vrne
# njegov `n`-ti odvod. Privzeta vrednost za `n` naj bo 1.
# 
...
#     []
# =============================================================================
def odvod_polinoma(polinom,n=1):
    sez=[]
...
# =====================================================================@001368=
# 3. podnaloga
# Napišite funkcije `vsota_polinomov(polinom1, polinom2)`, ki vrne vsoto danih
# polinomov.
# 
...
#     []
# =============================================================================
def vsota_polinomov(polinom1,polinom2):
    sez=[]
    for i in range(max(len(polinom1),len(polinom2))):
        c = polinom1[i] + polinom2[i]
        sez.append(c)
    return sez
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            Check.equal('odstrani_odvecne_nicle([0, 1, 2])', [0, 1, 2])
            Check.equal('odstrani_odvecne_nicle([2, 1, 0])', [2, 1])
            Check.equal('odstrani_odvecne_nicle([0, 0, 0, 0, 0, 0])', [])
            for n in range(100):
                polinom = n * [0, 1] + (n % 17) * [0]
                Check.secret(odstrani_odvecne_nicle(polinom))
        except:
    ...
    if Check.part():
        try:
            Check.equal('odvod_polinoma([1, 2, 3])', [2, 6])
            Check.equal('odvod_polinoma([4, -1, 2, 0, 1], n=2)', [4, 0, 12])
            Check.equal('odvod_polinoma([1])', [])
            for n in range(100):
                polinom = (n % 17) * [0, 1]
                Check.secret(odvod_polinoma(polinom, n % 6), (polinom, n % 6))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    if Check.part():
        try:
            Check.equal('vsota_polinomov([1, 2, 3], [5, 6, 7, 8])', [6, 8, 10, 8])
            Check.equal('vsota_polinomov([1, 2, 3], [])', [1, 2, 3])
            Check.equal('vsota_polinomov([1, 2, 3], [-1, -2, -3])', [])
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
