...
def pravokotnik(krogi):
    ...
    for i in range(len(krogi)):
        minimumi_x.append(krogi[i][0]-krogi[i][2])
        maximumi_x.append(krogi[i][0]+krogi[i][2])
        minimumi_y.append(krogi[i][1]-krogi[i][2])
        maximumi_y.append(krogi[i][1]+krogi[i][2])

    x_min=min(minimumi_x)
    x_max=max(maximumi_x)
    y_min=min(minimumi_y)
    y_max=max(maximumi_y)

    sez=(x_min,y_min,x_max,y_max)
