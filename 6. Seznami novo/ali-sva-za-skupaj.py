...
def ujemanje(oseba1,oseba2,geslo='LOVES'):
    sez1 = prestej_crke(geslo,oseba1)
    sez2 = prestej_crke(geslo,oseba2)
    sez=[]
    for i in range(len(sez1)):
        vsota=sez1[i]+sez2[i]
        sez.append(vsota)
    while len(sez)>2:
        sez = sestej_stevke(sez)
    return 10*sez[0]+sez[1]
...
def _validate_current_file():
    ...
    if Check.part():
        try:
            Check.equal('razbij_na_stevke(12382)', [1, 2, 3, 8, 2])
    ...
    if Check.part():
        try:
            Check.equal("prestej_crke('LOVES', 'france')", [0, 0, 0, 1, 0])
    ...
    if Check.part():
        try:
            Check.equal('sestej_stevke([1, 0, 4, 4])', [1, 4, 8])
            Check.equal('sestej_stevke([1, 4, 8])', [5, 1, 2])
    ...
    if Check.part():
        try:
            Check.equal("ujemanje('Julija Primic', 'France Prešeren')", 63)
            Check.equal("ujemanje('Julija Primic', 'France Prešeren', geslo='LOVES')", 63)
